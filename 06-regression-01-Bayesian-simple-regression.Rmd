...
$$ \widehat{\text{Bodyfat}} = -39.28 + 0.63\text{Abdomen}. $$
...
plot(residuals(bodyfat.lm) ~ fitted(bodyfat.lm), xlab = "fitted values", ylab = "residuals")
abline(h = 0)
```

With the exception of the one observation for the individual with the largest waist measurement, the residual plot suggests that this linear regression is a reasonable approximation.

Furthermore, we can check the normal probability plot of the residuals for the assumption of normally distributed errors. We see that only Case 39 is exceptionally away from the normal quantile. 
```{r out.width="70%", fig.align="center"}
...
$\text{se}_{\hat{\alpha}}$ and $\text{se}_{\hat{\beta}}$ are given as
$$ 
\begin{aligned}
...
\text{se}_{\hat{\beta}} = & \sqrt{\frac{\text{SSE}}{n-2}\frac{1}{\text{S}_xx}} = \frac{\hat{\sigma}}{\sqrt{\text{S}_{xx}}}.
\end{aligned}
$$

We may construct the confidence intervals of $\alpha$ and $\beta$ using the $t$-statistics
$$ 
t_\alpha^\ast = \frac{\alpha - \hat{\alpha}}{\text{se}_{\hat{\alpha}}},\qquad t_\beta^\ast = \frac{\beta-\hat{\beta}}{\text{se}_{\hat{\beta}}}.
$$

...
```{r BF-NC-plot, echo=FALSE, warning=FALSE, fig.cap="Estimates of effect under H2", fig.align='center', fig.height=2}
library(ggplot2)
out = bayes_inference(y=weight, x=habit, data=nc,type='ht',
 ...
 d = data.frame(mu = den$x, dens = den$y)
    ...
    pos_plot = ggplot(d, aes_string(x="mu", y="dens")) +
               geom_line() +
               ylab("Density") +
               xlab(expression(mu[nonsmoker] - mu[smoker])) +

...
\end{aligned}
$$
Any model can be used as the base model $M_b$. It could be the model with the highest posterior probability, or the one with just the intercept $y_i = \beta_0+\epsilon_i$. 
...
`BF`          | Bayes factor $\text{BF}[M_m:M_b]$, where $M_b$ is the model with highest posterior probability

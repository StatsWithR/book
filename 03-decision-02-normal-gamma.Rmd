...
```{r plot-post-pred, echo=FALSE, message=FALSE, warning=FALSE, fig.align="center", fig.height=5, fig.cap="Comparison of posterior densities"}
library(ggplot2)
...
cauchy.plot = ggplot(data=df, aes(x=x,y=y, group=distribution)) +
         geom_line(aes(linetype=distribution, colour=distribution), size=1.5) + xlab("y") + ylab("density") + scale_colour_manual(values=cbPalette) +
            theme(panel.background = element_rect(fill = "transparent", colour = NA),
                  legend.key = element_rect(colour = "transparent", fill = NA),
                  plot.background = element_rect(fill = "transparent", colour = NA)) +
            theme(legend.position=c(.75, .75)) +
...
#               rep("ref posterior mu", nsim), rep("posterior predictive Y", nsim),
               rep("JZS posterior mu", nsim), rep("JZS posterior predictive", nsim)
                ),


  x = c(mu, y,  #post_mu, pred_y,
        mu.cauchy, pred_y_cauchy)
)
```
...
postpred =  ggplot(data=df, aes(x=pred_y)) +
             geom_density(aes(x=x, colour=parameter, linetype=parameter), show_guide=FALSE, size=1.2)+
            stat_density(aes(x=x, colour=parameter, linetype=parameter),

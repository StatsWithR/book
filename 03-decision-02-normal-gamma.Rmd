...
```
Prior $\textsf{NormalGamma}(`r m_0`, `r n_0`, `r s2_0`, `r v_0`)$ 

Data  $\bar{Y} = `r ybar`$, $s^2 = `r s2`$, $n = `r n`$

\begin{eqnarray*}
n_n & = &  `r n_0` +  `r n` = `r n_n`\\
m_n  & = & \frac{`r n` \times`r ybar` + `r n_0` \times`r m_0`}{`r n_n`} = `r m_n`  \\
v_n & = & `r v_0` + `r n` = `r v_n`  \\
s^2_n & = & \frac{(n-1) s^2 + v_0 s^2_0 + n_0 n (m_0 - \bar{Y})^2 /n_n }{v_n}  \\
  & = & \frac{1}{`r v_n`}
     \left[`r n-1` \times `r s2` +
          `r v_0` \times `r s2_0`  +
          \frac{`r n_0` \times `r n`}{`r n_n`} \times (`r m_0` - `r ybar`)^2
\right] = `r round(s2_n, 1)`  \\
\end{eqnarray*}
Posterior $\textsf{NormalGamma}(`r m_n`, `r n_n`, `r s2_n`, `r v_n`)$

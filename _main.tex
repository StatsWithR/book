...
\hypersetup{unicode=true,
            ...
            pdfauthor={Christine Chai; Merlise Clyde; Lizzy Huang; Colin Rundel},
            pdfsubject={A Companion to the Statistics with R Coursera Course},
            pdfborder={0 0 0},
            breaklinks=true}
...
\protect\hypertarget{exm:unnamed-chunk-2}{}\label{exm:unnamed-chunk-2} What
is the probability that an 18-29 year old from Table
...
\BeginKnitrBlock{example}
\protect\hypertarget{exm:RU-486}{}\label{exm:RU-486} RU-486 is claimed to be
an effective ``morning after'' contraceptive pill, but is it really
effective?
...
\BeginKnitrBlock{example}
\protect\hypertarget{exm:MM}{}\label{exm:MM} We have a population of M\&M's,
and in this population the percentage of yellow M\&M's is either 10\% or
20\%. You've been hired as a statistical consultant to decide whether
the true percentage of yellow M\&M's is 10\% or 20\%.
...
\NormalTok{m_0 =}\StringTok{ }\DecValTok{35}\NormalTok{; n_0 =}\StringTok{ }\DecValTok{25}\NormalTok{;  s2_0 =}\StringTok{ }\FloatTok{156.25}\NormalTok{; v_0 =}\StringTok{ }\NormalTok{n_0 -}\StringTok{ }\DecValTok{1}
\CommentTok{# sample summaries}
\NormalTok{Y =}\StringTok{ }\NormalTok{tapwater$tthm}
...
\NormalTok{n_n =}\StringTok{ }\NormalTok{n_0 +}\StringTok{ }\NormalTok{n}
\NormalTok{m_n =}\StringTok{ }\NormalTok{(n*ybar +}\StringTok{ }\NormalTok{n_0*m_0)/n_n}
\NormalTok{v_n =}\StringTok{ }\NormalTok{v_0 +}\StringTok{ }\NormalTok{n}
\NormalTok{s2_n =}\StringTok{ }\NormalTok{((n}\DecValTok{-1}\NormalTok{)*s2 +}\StringTok{ }\NormalTok{v_0*s2_0 +}\StringTok{ }\NormalTok{n_0*n*(m_0 -}\StringTok{ }\NormalTok{ybar)^}\DecValTok{2}\NormalTok{/n_n)/v_n}
\end{Highlighting}
\end{Shaded}
...
\includegraphics{04-normalgamma-01-inference_files/figure-latex/tapwater-post-mu-1.pdf}
...
\NormalTok{m_0 =}\StringTok{ }\DecValTok{35}\NormalTok{; }
\NormalTok{n_0 =}\StringTok{ }\DecValTok{25}\NormalTok{; }
\NormalTok{s2_0 =}\StringTok{ }\FloatTok{156.25}\NormalTok{;}
\NormalTok{v_0 =}\StringTok{ }\NormalTok{n_0 -}\StringTok{ }\DecValTok{1}
\NormalTok{Y =}\StringTok{ }\NormalTok{tapwater$tthm}
\NormalTok{ybar =}\StringTok{ }\KeywordTok{mean}\NormalTok{(Y)}
\NormalTok{s2 =}\StringTok{ }\KeywordTok{round}\NormalTok{(}\KeywordTok{var}\NormalTok{(Y),}\DecValTok{1}\NormalTok{)}
\NormalTok{n =}\StringTok{ }\KeywordTok{length}\NormalTok{(Y)}
\NormalTok{n_n =}\StringTok{ }\NormalTok{n_0 +}\StringTok{ }\NormalTok{n}
\NormalTok{m_n =}\StringTok{ }\KeywordTok{round}\NormalTok{((n*ybar +}\StringTok{ }\NormalTok{n_0*m_0)/n_n, }\DecValTok{1}\NormalTok{)}
\NormalTok{v_n =}\StringTok{ }\NormalTok{v_0 +}\StringTok{ }\NormalTok{n}
\NormalTok{s2_n =}\StringTok{ }\KeywordTok{round}\NormalTok{( ((n}\DecValTok{-1}\NormalTok{)*s2 +}\StringTok{ }\NormalTok{v_0*s2_0 +}\StringTok{ }\NormalTok{n_0*n*(m_0 -}\StringTok{ }\NormalTok{ybar)^}\DecValTok{2}\NormalTok{/n_n)/v_n, }\DecValTok{1}\NormalTok{)}
\NormalTok{L =}\StringTok{ }\KeywordTok{qt}\NormalTok{(.}\DecValTok{025}\NormalTok{, v_n)*}\KeywordTok{sqrt}\NormalTok{(s2_n/n_n) +}\StringTok{ }\NormalTok{m_n}
\NormalTok{U =}\StringTok{ }\KeywordTok{qt}\NormalTok{(.}\DecValTok{975}\NormalTok{, v_n)*}\KeywordTok{sqrt}\NormalTok{(s2_n/n_n) +}\StringTok{ }\NormalTok{m_n}
\end{Highlighting}
\end{Shaded}
...
\includegraphics{04-normalgamma-06-MCMC_files/figure-latex/tapwater-inference-1.pdf}
...
\begin{figure}[htbp]
\centering
\includegraphics{05-BFnormal-01-known-variance_files/figure-latex/vague-prior-1.pdf}
...
\begin{figure}[htbp]
\centering
\includegraphics{05-BFnormal-01-known-variance_files/figure-latex/effect-size-1.pdf}
...
\begin{figure}[htbp]
\centering
\includegraphics{05-BFnormal-01-known-variance_files/figure-latex/prior-effect-1.pdf}
...
\includegraphics{05-BFnormal-02-paired-means_files/figure-latex/bayes-inference-1.pdf}
...
\includegraphics{06-regression-01-Bayesian-simple-regression_files/figure-latex/unnamed-chunk-1-1.pdf}
...
\includegraphics{06-regression-01-Bayesian-simple-regression_files/figure-latex/unnamed-chunk-2-1.pdf}
...
\includegraphics{06-regression-01-Bayesian-simple-regression_files/figure-latex/unnamed-chunk-3-1.pdf}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{output =}\StringTok{ }\KeywordTok{summary}\NormalTok{(bodyfat.lm)$coef[, }\DecValTok{1}\NormalTok{:}\DecValTok{2}\NormalTok{]}
\NormalTok{output}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{x =}\StringTok{ }\NormalTok{bodyfat$Abdomen}
\NormalTok{y =}\StringTok{ }\NormalTok{bodyfat$Bodyfat}
...
\KeywordTok{lines}\NormalTok{(ynew$lwr ~}\StringTok{ }\NormalTok{xnew, }\DataTypeTok{lty =} \DecValTok{2}\NormalTok{, }\DataTypeTok{lwd =} \DecValTok{3}\NormalTok{, }\DataTypeTok{col =} \StringTok{"darkgrey"}\NormalTok{)}
\KeywordTok{lines}\NormalTok{(ynew$upr ~}\StringTok{ }\NormalTok{xnew, }\DataTypeTok{lty =} \DecValTok{2}\NormalTok{, }\DataTypeTok{lwd =} \DecValTok{3}\NormalTok{, }\DataTypeTok{col =} \StringTok{"darkgrey"}\NormalTok{)}
...
\KeywordTok{lines}\NormalTok{(ypred$lwr ~}\StringTok{ }\NormalTok{xnew, }\DataTypeTok{lty =} \DecValTok{3}\NormalTok{, }\DataTypeTok{lwd =} \DecValTok{3}\NormalTok{, }\DataTypeTok{col =} \StringTok{"darkgrey"}\NormalTok{)}
\KeywordTok{lines}\NormalTok{(ypred$upr ~}\StringTok{ }\NormalTok{xnew, }\DataTypeTok{lty =} \DecValTok{3}\NormalTok{, }\DataTypeTok{lwd =} \DecValTok{3}\NormalTok{, }\DataTypeTok{col =} \StringTok{"darkgrey"}\NormalTok{)}
...
\includegraphics{06-regression-01-Bayesian-simple-regression_files/figure-latex/predict-intervals-1.pdf}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{pred}\FloatTok{.39} \NormalTok{=}\StringTok{ }\KeywordTok{predict}\NormalTok{(bodyfat.lm, }\DataTypeTok{newdata =} \NormalTok{bodyfat[}\DecValTok{39}\NormalTok{, ], }\DataTypeTok{interval =} \StringTok{"prediction"}\NormalTok{, }\DataTypeTok{level =} \FloatTok{0.95}\NormalTok{)}
\NormalTok{out <-}\StringTok{ }\KeywordTok{cbind}\NormalTok{(bodyfat[}\DecValTok{39}\NormalTok{,]$Abdomen, pred}\FloatTok{.39}\NormalTok{)}
...
The article by \citet{chaloner1988bayesian} suggested an approach for
defining outliers and then calculating the probability that a case or
multiple cases were outliers. The assumed model for our simple linear
regression is \(y_i=\alpha + \beta x_i+\epsilon_i\), with \(\epsilon_i\)
having independent, identical distributions that are normal with mean
zero and constant variance \(\sigma^2\), i.e.,
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{cognitive$mom_work <-}\StringTok{ }\KeywordTok{as.numeric}\NormalTok{(cognitive$mom_work >}\StringTok{ }\DecValTok{1}\NormalTok{)}
\NormalTok{cognitive$mom_hs <-}\StringTok{ }\KeywordTok{as.numeric}\NormalTok{(cognitive$mom_hs >}\StringTok{ }\DecValTok{0}\NormalTok{)}
...
\includegraphics{06-regression-03-Bayesian-multi-regression_files/figure-latex/plot-coef-1.pdf}

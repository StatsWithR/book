...
Here, $\sigma^2$, $S_\alpha$, $S_\beta$, and $S_{\alpha\beta}$ are hyperparameters. This is equivalent to setting the coefficient vector $\bv = (\alpha, \beta)^T$^[$(\alpha, \beta)^T$ means we transpose the row vector $(\alpha, \beta)$ into a column vector $\left(\begin{array}{c} \alpha \\ \beta \end{array}\right)$.] to have a biraviate normal distribution with convariance matrix $\Sigma_0$
$$ \Sigma_0 = \sigma^2\left(\begin{array}{cc} S_\alpha & S_{\alpha\beta} \\
S_{\alpha\beta} & S_\beta \end{array} \right). $$
That is,
$$ \bv = (\alpha, \beta)^T ~|~\sigma^2 \sim \textsf{BiraviateNormal}(\mathbf{b} = (a_0, b_0)^T, \sigma^2\Sigma_0). $$
...
The marginal posterior distribution of the coefficient vector $\bv = (\alpha, \beta)$ will be biraviate normal, and the marginal posterior distribution of $\sigma^2$ is again an inverse Gamma distribution
